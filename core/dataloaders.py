...
class DataLoader:
	...
	def _load_from_file(self, file_path, dtype=float):
		...
		with open(file_path, "r") as fp:
			if self.use_mmap:
				...
				func_load_line = partial(self._load_line, delimiter=b",")
			else:
			...
			self.idx = list(range(N_lines_selected))
	...
	def __iter__(self):
		...
		for i in range(0, self.N_to_train, self.batch_size):
			yield self.X[self.idx[i: i + self.batch_size]], \
			      self.Y[self.idx[i: i + self.batch_size]]
